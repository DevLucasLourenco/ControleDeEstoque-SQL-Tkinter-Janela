...
def adicionar_insumo():
    ...
    caixa_texto.insert("1.0", f"{nome_insumo.get()} adicinado com sucesso!")
    
    
    
def deletar_insumo():
    # ver se tem texto no nome_insumo
    if len(nome_insumo.get()) < 2:
        # deletar o que está escrito na caixa de texto
        caixa_texto.delete('1.0',END)
        # escrever na caixa de texto
    ...
    cursor.execute(f'''
              ...
              WHERE Produto="{nome_insumo.get()}"
                   ''')
    cursor.commit() #para perpetuar a edição no BD
...
def consumir_insumo():
    if len(nome_insumo.get()) < 2 or len(lote_insumo.get()) < 1 or len(qtde_insumo.get()) < 1:
        ...
        # escrever na caixa de texto
        caixa_texto.insert("1.0", "nome e lote e quantidade do insumo inválidos!")
        return
        ...
        # escrever na caixa de texto
        caixa_texto.insert("1.0", f"O nome {nome_insumo.get()} é inválido")

def visualizar_insumo():
    if len(nome_insumo.get()) == 0:
        ...
        table.columns.header = ['Nome','Quantidade','Validade','Lote']
        for id_produto, nome, quantidade, validade, lote in valores:
            table.rows.append([nome, quantidade, validade, lote])
        
        caixa_texto.delete('1.0',END)
        caixa_texto.insert('1.0', table)
